...
function setup() {
  buildScrollMap();
  spacer.style.height = TOTAL_SCROLL + 'px';
  document.body.style.height = TOTAL_SCROLL + 'px';
}
...
function buildScrollMap() {
  ...
  for (let i = 0; i < SCENES; i++) {
    ...
    // Scene 21 (i=10) has margin-left: 65vw in CSS, so its strip position is offset by 0.65 * vw
    const stripX = i * vw + (i === 10 ? 0.65 * vw : 0);
...
function frame(ts) {
  ...
  if (currentScene === 7 && SCROLL_MAP[7]) {
    ...
    effectiveTx = -(SCROLL_MAP[9].stripX + 0.37 * _vw);
  } else if (currentScene === 10 && SCROLL_MAP[10]) {
    // Scene 21: pan 300vw across the 400vw road (left edge → right edge over full scene)
    effectiveTx = -(SCROLL_MAP[10].stripX + sceneLocal * 3 * _vw);
  } else {
    effectiveTx = tx;
  }
  ...
  const busVw      = _inJungle
    ...
    : '–';
  const SCENE_LABELS = [1,2,3,4,5,6,7,8,12,13,21];
  ...
  if (panels[8]) {
    const show8 = currentScene === 7 && sceneLocal > 0.04 && sceneLocal < 0.16;
...
function animateMatatu(scene, local, tx, junglePhase, opacity) {
  ...
  if (jungleBus) {
    if (inJungle) {
      ...
      let busOpacityLocal;
      if (scene === 0) {
        // Scene 1: easeInOut — zero velocity at start and stop, no jerk
...
function animateCityBus(scene, local, opacity) {
  ...
  if (scene === 3) {
    ...
    // Scene 5: bus continues from ENTRY (left edge) to CENTER — already visible, no fade
    const t = easeInOutCubic(Math.min(1, local / 0.5));
    busX = ENTRY + t * (CENTER - ENTRY);
    eff  = opacity;
  } else if (scene === 5) {
    eff = opacity;
